...
def _build_syl(vowels, tone_numbers=False):
    ...
    consonant_end = "(?![%(a)s%(e)s%(i)s%(o)s%(u)s%(v)s]|u:)" % {"a": _a, "e": _e, "i": _i, "o": _o, "u": _u, "v": _v}
    _vowels = vowels.copy()
...
def _build_word(syl, vowels):
    ...
    """
    return ("(?:%(syl)s(?:-(?=%(syl)s)|'(?=[%(a)s%(e)s%(o)s])(?=%(syl)s))?)+") % {
        "syl": syl,
        "a": vowels["a"],
        "e": vowels["e"],
        "o": vowels["o"],
...
def _build_sentence(word):
    ...
    """
    return ("""(?:%(word)s|[%(non_stops)s]|(?<![%(stops)s ]) )+""" """[%(stops)s]['"\]\}\)]*""") % {
        "word": word,
        "non_stops": non_stops.replace("-", "\-"),
        "stops": stops,
    }
